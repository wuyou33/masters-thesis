...
\begin{tikzpicture}[->,>=stealth',node distance=1.5cm]
    ...
    \begin{pgfonlayer}{background}
		...
		\path[fill=gray!20,rounded corners, draw=black!50]
            (a) rectangle (b);
    \end{pgfonlayer}
...
The software on xMega consists of 4 different tasks running equally in parallel. There is \textbf{CommTask} which is designated to handle all incoming and outgoing communication. Since 	sharing access to all communication media would be difficult to maintain, all other tasks communicate with peripherals via the CommTask. Another one is \textbf{ControllersTask} which handles computations with regards to system controllers. \textbf{LogTask} is responsible for periodic data logging to OpenLog module. And despite its name, \textbf{MainTask} handles minor jobs as trajectory following and flight state automaton (modeling different flight modes of the UAV). 

\subsection{Tasks on STM MCU}

Tasks on STM has much clearer designation. There is also the \textbf{CommTask} serving as a communication mediator. The another one is \textbf{KalmanTask}. It handles the complete computation of the Kalman filter. Finally the last one is \textbf{MPCTask} that takes care of computing the model predictive controller. 
...
In order to implement the Kalman filter (section \ref{cap:kalman_filter_theory}) and the model predictive controller (section \ref{cap:mpc_theory}) one has to deal with a lot of vector and matrix operations. Although one can implement them directly using the programming language it is impractical for code debugging and it discards the clarity of mathematical matrix notation. There is a large selection of matrix libraries \citep{matrixlibraries} available for variety of programming languages. But since both our code projects are developed using ANSI C programming language and our platforms are very specific (the library should be supplied by means of source code) we decided to develop our own matrix library. Another reason that supports this decision is to maintain as mmuch control over the executing code as possible, specifically with regards to memory allocations, which is a critical issue on micro-controllers. Our library supports basic matrix and vector operation with floating point values as well as basic algebra operators --- matrix inversion and computation of determinant. It was developed with regard to minimize stack memory footprint by requiring prior allocation of memory for subresults. There is also a possibility to create a matrix using a constant data located within a program memory. 

CMatrixLib, as it called, is presented on a community website GitHub \citep{cmatrixlib} under GNU General Public License. As it is with a proper documentation.
